...
def generate_pdf(doc_type, ref, po_number, date, loads, liters, rate, subtotal, vat, total):
    ...
    # Table - use ReportLab Table for proper alignment
    table_top = height - 155*mm
    
    # Column x positions and widths (all fit within margins)
    margin = 15*mm
    table_width = width - 2*margin
    
    # Col positions: Item | Description | Quantity | Unit Price | Total Price
    col_x = [margin, margin+12*mm, margin+100*mm, margin+128*mm, margin+152*mm]
    col_right_edge = width - margin  # right edge for right-aligned values
    row_h = 8*mm
    ...
    c.setFillColor(colors.black)
    c.setFont("Helvetica-Bold", 9)
    c.drawString(col_x[0]+1*mm, table_top+2.5*mm, "Item")
    c.drawString(col_x[1]+1*mm, table_top+2.5*mm, "Description")
    c.drawString(col_x[2]+1*mm, table_top+2.5*mm, "Quantity")
    c.drawString(col_x[3]+1*mm, table_top+2.5*mm, "Unit Price")
    c.drawRightString(col_right_edge-1*mm, table_top+2.5*mm, "Total Price")
    ...
    c.setFillColor(colors.black)
    c.setFont("Helvetica", 9)
    c.drawString(col_x[0]+1*mm, row_y+2.5*mm, "1")
    c.drawString(col_x[1]+1*mm, row_y+2.5*mm, f"Supply of Water ({date.strftime('%d %B %Y')})")
    c.drawString(col_x[2]+1*mm, row_y+2.5*mm, f"{liters:,.0f} Ltrs")
    c.drawString(col_x[3]+1*mm, row_y+2.5*mm, f"R{rate:.2f}")
    c.drawRightString(col_right_edge-1*mm, row_y+2.5*mm, f"R{subtotal:,.2f}")
    
    # Totals - right aligned
    totals_y = row_y - 10*mm
    label_x = col_x[3]+1*mm
    
    c.setFont("Helvetica", 9)
    c.drawString(label_x, totals_y, "Sub-total")
    c.drawRightString(col_right_edge-1*mm, totals_y, f"R{subtotal:,.2f}")
    
    totals_y -= 6*mm
    c.setFillColor(colors.blue)
    c.drawString(label_x, totals_y, "VAT@15%")
    c.setFillColor(colors.black)
    c.drawRightString(col_right_edge-1*mm, totals_y, f"R{vat:,.2f}")
    
    totals_y -= 6*mm
    c.setFont("Helvetica-Bold", 9)
    c.drawString(label_x, totals_y, "Grand Total")
    c.drawRightString(col_right_edge-1*mm, totals_y, f"R{total:,.2f}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Always authorize the first user or the env-set user
    authorized = os.environ.get('AUTHORIZED_USER_ID')
    if authorized:
        state["authorized_user"] = int(authorized)
        save_state(state)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Auto-authorize if env variable is set
    authorized = os.environ.get('AUTHORIZED_USER_ID')
    if authorized:
        state["authorized_user"] = int(authorized)
        save_state(state)
